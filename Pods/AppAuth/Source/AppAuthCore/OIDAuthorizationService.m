...
/*
 *  validateScheme:
 *      function that inspects uri redirect url based on scheme
 *      if the uri starts with scheme:// the redirect url is considered as valid
 */
+ (BOOL)validateScheme:(NSURL *)standardizedURL
                scheme:(NSString *)scheme {
    NSString *pattern = [NSString stringWithFormat:@"%@://", [scheme lowercaseString]];
    return [[standardizedURL absoluteString] hasPrefix:pattern];
}

/*
 *  handlePlistScheme:
 *      iterates through info.plist for uri schemes
 *      if defined schema matches beginning of redirect-uri then accept
 */
+ (BOOL)handlePlistScheme:(NSURL *)URL {
    NSURL *standardizedURL = [URL standardizedURL];
        ...
        for (int i = 0; i < [URLTypes count]; i++) {
            ...
            if (schemes != nil) {
                for (int j = 0; j < [schemes count]; j++) {
                    if ([OIDAuthorizationSession validateScheme:standardizedURL
                                                         scheme:[schemes objectAtIndex:j]
                        ]) {
                        return TRUE;
                    }
                }
...
- (BOOL)shouldHandleURL:(NSURL *)URL {
    if ([OIDAuthorizationSession handlePlistScheme:URL])
        return TRUE;
